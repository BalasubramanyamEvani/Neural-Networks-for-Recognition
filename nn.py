import numpy as np
from util import *

# do not include any more libraries here!
# do not put any code outside of functions!


############################## Q 2.1 ##############################
# initialize b to 0 vector
# b should be a 1D array, not a 2D array with a singleton dimension
# we will do XW + b.
# X be [Examples, Dimensions]
def initialize_weights(in_size, out_size, params, name=""):
    W, b = None, None
    b = np.zeros((out_size,))
    t = np.sqrt(6) / np.sqrt(in_size + out_size)
    W = np.random.uniform(low=-t, high=t, size=(in_size, out_size))
    params["W" + name] = W
    params["b" + name] = b


############################## Q 2.2.1 ##############################
# x is a matrix
# a sigmoid activation function
def sigmoid(x):
    res = None
    with np.errstate(invalid="ignore", over="ignore"):
        res = np.where(x < 0, np.exp(x) / (1 + np.exp(x)), 1 / (1 + np.exp(-x)))
    return res


############################## Q 2.2.1 ##############################
def forward(X, params, name="", activation=sigmoid):
    """
    Do a forward pass

    Keyword arguments:
    X -- input vector [Examples x D]
    params -- a dictionary containing parameters
    name -- name of the layer
    activation -- the activation function (default is sigmoid)
    """
    pre_act, post_act = None, None
    # get the layer parameters
    W = params["W" + name]
    b = params["b" + name]

    pre_act = X @ W + b
    post_act = activation(pre_act)

    # store the pre-activation and post-activation values
    # these will be important in backprop
    params["cache_" + name] = (X, pre_act, post_act)

    return post_act


############################## Q 2.2.2  ##############################
# x is [examples,classes]
# softmax should be done for each row
def softmax(x):
    e_x = np.exp(x)
    res = e_x / (np.sum(e_x, axis=1))[:, np.newaxis]
    return res


############################## Q 2.2.3 ##############################
# compute total loss and accuracy
# y is size [examples,classes]
# probs is size [examples,classes]
def compute_loss_and_acc(y, probs):
    true_labels = np.argmax(y, axis=1)
    pred_labels = np.argmax(probs, axis=1)
    num_labels = list(range(y.shape[0]))
    loss, acc = (
        np.sum(-np.log(probs)[num_labels, true_labels]),
        np.sum(true_labels == pred_labels) / y.shape[0],
    )
    return loss, acc


############################## Q 2.3 ##############################
# we give this to you
# because you proved it
# it's a function of post_act
def sigmoid_deriv(post_act):
    res = post_act * (1.0 - post_act)
    return res


def backwards(delta, params, name="", activation_deriv=sigmoid_deriv):
    """
    Do a backwards pass

    Keyword arguments:
    delta -- errors to backprop
    params -- a dictionary containing parameters
    name -- name of the layer
    activation_deriv -- the derivative of the activation_func
    """
    grad_X, grad_W, grad_b = None, None, None
    # everything you may need for this layer
    W = params["W" + name]
    b = params["b" + name]
    X, pre_act, post_act = params["cache_" + name]

    # do the derivative through activation first
    # (don't forget activation_deriv is a function of post_act)
    # then compute the derivative W, b, and X
    grad_b = np.zeros_like(b)
    grad_W = np.zeros_like(W)
    grad_X = np.zeros_like(X)

    grad_W = X.T @ (delta * activation_deriv(post_act))
    grad_b = (np.ones((1, X.shape[0])) @ (delta * activation_deriv(post_act))).squeeze(
        axis=0
    )
    grad_X = delta * activation_deriv(post_act) @ W.T

    # store the gradients
    params["grad_W" + name] = grad_W
    params["grad_b" + name] = grad_b
    return grad_X


############################## Q 2.4 ##############################
# split x and y into random batches
# return a list of [(batch1_x,batch1_y)...]
def get_random_batches(x, y, batch_size):
    batches = []
    n = x.shape[0]
    indices = np.random.permutation(list(range(n)))
    num_batches = (n // batch_size) + (1 if n % batch_size != 0 else 0)
    for i in range(num_batches):
        start = batch_size * i
        end = batch_size * (i + 1)
        if end < len(indices):
            ind = indices[start:end]
        else:
            ind = indices[start:]

        x_batch = x[ind, :]
        y_batch = y[ind]
        batches.append((x_batch, y_batch))

    return batches
