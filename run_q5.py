...
params["m_Wlayer1"] = np.zeros((1024, hidden_size))
params["m_blayer1"] = np.zeros(hidden_size)

params["m_Wlayer2"] = np.zeros((hidden_size, hidden_size))
params["m_blayer2"] = np.zeros(hidden_size)

params["m_Wlayer3"] = np.zeros((hidden_size, hidden_size))
params["m_blayer3"] = np.zeros(hidden_size)

params["m_Woutput"] = np.zeros((hidden_size, 1024))
params["m_boutput"] = np.zeros(1024)
...
for itr in range(max_iters):
    ...
    for xb, _ in batches:
        ...
        for k, v in sorted(list(params.items())):
            if "grad" in k:
                name = k.split("_")[1]
                params[f"m_{name}"] = 0.9 * params[f"m_{name}"] - learning_rate * v
                params[name] += params[f"m_{name}"]

    losses.append(total_loss / train_x.shape[0])
    if itr % 2 == 0:
        print("itr: {:02d} \t loss: {:.2f}".format(itr, losses[-1]))
...
for i, label in enumerate(visualize_labels):
    idx = (
        26 + int(label)
        if label.isnumeric()
        else string.ascii_lowercase.index(label.lower())
    )
...
reconstructed_x = forward(h3, params, "output", sigmoid)
psnr = 0.0
num_val_imgs = len(valid_x)
for i in range(num_val_imgs):
    val, reconstructed = valid_x[0], reconstructed_x[0]
    psnr += peak_signal_noise_ratio(val, reconstructed)
print(f"Avg. PSNR: {psnr / num_val_imgs}")
